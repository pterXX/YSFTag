...
- (void)onTag:(YSFTagButton *)btn{
    if (self.tagViewStyle == YSFTagViewStyleDefault) {
        
    }
    if(self.tagViewStyle == YSFTagViewStyleOnlySelectOne){
        btn.selected = YES;
        [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YSFTagButton *bnt = (YSFTagButton *)obj;
            if (bnt != btn) {
                bnt.selected = NO;
    ...
    if (self.tagViewStyle == YSFTagViewStyleCanSelectMore) {
...
- (void)addTag:(YSFTag *)tag {
    NSParameterAssert(tag);
    YSFTagButton *btn = [YSFTagButton buttonWithTag:tag];
...
- (void)insertTag:(YSFTag *)tag atIndex:(NSUInteger)index {
    NSParameterAssert(tag);
